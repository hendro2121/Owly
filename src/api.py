...
app = FastAPI(title="Owly API", description="JSE Insider Trading Intelligence", version="0.2.0", docs_url="/docs")
...
APP_URL = os.environ.get("APP_URL", "https://owly-production-ca3b.up.railway.app")
...
async def startup():
    ...
    logger.info("Owly API started (PostgreSQL + daily Sharenet scraper)")
...
async def get_sens_pdf(deal_id: int = Path(..., ge=1)):
    ...
    # Header
    elements.append(Paragraph("OWLY", style_header))
    ...
    elements.append(Paragraph(f"Generated by Owly — Source: {src}", style_footer))
...
if os.path.isdir(STATIC_DIR):
    ...
    SPA_FILE = os.path.join(STATIC_DIR, "index.html")

    def _landing_or_spa():
        return FileResponse(LANDING_FILE) if os.path.isfile(LANDING_FILE) else FileResponse(SPA_FILE)

    # "/" serves the standalone marketing landing; the React app lives at /app.
    # Declared before the catch-all below — FastAPI matches routes in order.
    @app.get("/")
    async def serve_landing():
        return _landing_or_spa()
    ...
    async def serve_frontend(path: str):
        ...
        return FileResponse(fp) if os.path.isfile(fp) else _landing_or_spa()
